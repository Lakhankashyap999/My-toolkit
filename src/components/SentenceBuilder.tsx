...
import { CheckCircle2, AlertTriangle, RotateCcw, Sparkles, ShieldCheck } from 'lucide-react';
...
export default function SentenceBuilder() {
  ...
  const handleCheck = () => {
    ...
    if (verbPosition !== 1) {
      // Not at index 1 (which is 2nd position)
      setStatus('wrong_v2');
      return;
    }
  ...
  return (
    <div className="mx-auto w-full max-w-xl rounded-3xl border border-neutral-200 bg-white p-6 sm:p-8 shadow-xl dark:border-neutral-800 dark:bg-neutral-900">
      {/* Header */}
      ...
      {/* Slots / Drop Zone */}
      <div className="min-h-[70px] rounded-2xl border-2 border-dashed border-neutral-300 bg-neutral-50/50 p-3 flex flex-wrap items-center gap-2 mb-6 dark:border-neutral-700 dark:bg-neutral-950/40">
        {selectedTokens.length === 0 ? (
          <span className="text-xs text-neutral-400 font-medium italic mx-auto">
            Niche diye shabdon par click karke sahi kram mein sajaiye...
          </span>
        ) : (
          selectedTokens.map((token, index) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 rounded-xl px-3.5 py-2 text-sm font-bold shadow-sm transition-all animate-in zoom-in-95 ${
                  isVerb
              ...
              >
                ...
                <span>{token}</span>
                {isVerb && (
                  <span className="rounded-full bg-rose-200 px-1 text-[9px] font-black text-rose-800 dark:bg-rose-900">
                    VERB
                  </span>
                )}
              </button>
      ...
      <div className="mb-6">
        <p className="text-xs font-semibold text-neutral-400 mb-2">Available Word Chips:</p>
          ...
          {availableTokens.map((token) => {
            ...
            return (
              <button
                ...
                className={`rounded-xl px-4 py-2.5 text-sm font-bold shadow-sm border transition-all active:scale-95 ${
                  isVerb
      ...
      {/* Feedback Alerts */}
      {status === 'wrong_v2' && (
        <div className="mb-4 rounded-2xl border border-rose-300 bg-rose-50 p-4 text-xs font-semibold text-rose-900 dark:border-rose-900 dark:bg-rose-950/40 dark:text-rose-200 animate-shake">
      ...
      {status !== 'correct' ? (
        <button
          ...
          className="w-full rounded-2xl bg-neutral-900 py-3.5 text-sm font-bold text-white shadow-md hover:bg-neutral-800 active:scale-98 disabled:opacity-40 dark:bg-white dark:text-neutral-900"
        >
        ...
        <button
          onClick={handleNext}
          className="w-full rounded-2xl bg-emerald-600 py-3.5 text-sm font-bold text-white shadow-md hover:bg-emerald-700 active:scale-98"
        >
          Next Sentence ➔
        </button>
      )}
